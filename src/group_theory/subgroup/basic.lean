...
class inv_mem_class (S G : Type*) [has_inv G] extends set_like S G :=
...
class neg_mem_class (S G : Type*) [has_neg G] extends set_like S G :=
...
class subgroup_class (S G : Type*) [group G]
  extends submonoid_class S G :=
(inv_mem : ∀ {s : S} {x}, x ∈ s → x⁻¹ ∈ s)

/-- `add_subgroup_class S G` states `S` is a type of subsets `s ⊆ G` that are
additive subgroups of `G`. -/
class add_subgroup_class (S G : Type*) [add_group G]
  extends add_submonoid_class S G :=
(neg_mem : ∀ {s : S} {x}, x ∈ s → -x ∈ s)

attribute [to_additive] inv_mem_class subgroup_class

variables (S : Type*) (G) [group G] [hSG : subgroup_class S G]
include hSG

@[to_additive] instance subgroup_class.to_inv_mem_class : inv_mem_class S G :=
{ .. hSG }
...
instance to_comm_group {G : Type*} [comm_group G] [subgroup_class S G] : comm_group H :=
...
instance to_ordered_comm_group {G : Type*} [ordered_comm_group G] [subgroup_class S G] :
...
subtype.rec_on x $ λ x hx, begin
  ...
  exact closure_induction hx
    ...
    (λ x hx, exists.elim hx $ λ hx' hx, ⟨inv_mem hx', Hinv _ hx⟩),
end
