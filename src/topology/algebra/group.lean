...
@[simp, to_additive] lemma to_topological_space_top : (⊤ : group_topology α).to_topological_space = ⊤ := rfl

@[to_additive]
instance : order_top (group_topology α) :=
{ top := ⊤,
  top_le := λ x, to_topological_space_le.2 $ top_le _ }
